...
## Onboarding checklist for new maintainers (architects & devs)

1. Read [CLAUDE.md](CLAUDE.md) for project goals and phase status.
2. Run `pnpm bootstrap` and `pnpm dev` to bring up the app locally.
3. Run `pnpm test` and `pnpm test:e2e` to verify the environment.
4. Inspect `packages/content-schema` and `content/` to understand curriculum structure.
5. Review `scripts/generate-pages.ts` and `scripts/generate-tests.ts` to understand code generation and the field-inference heuristics.
6. Review the new content repository abstraction in `packages/learning-engine` and the admin content paths in `apps/server`.
7. Review DB schema in `packages/db/prisma/schema.prisma` and seed scripts.

High-priority improvements (recommended):
- Add deterministic test seeding helpers to improve generated API test stubs.
- Add automated accessibility checks to CI.
- Add deployment + backup runbook (`DEPLOYMENT.md`) with DB backup/restore steps.
- Add dependency vulnerability scanning step in CI.
- Verify Admin Content Studio flow and admin OpenAPI surface.

---

## Deployment and ops (summary)

- Recommended production deployment: containerized apps (Docker) behind an HTTPS load balancer. Store secrets in a secure secrets manager and use managed MySQL or an equivalent with daily backups.
- The content platform now uses MySQL-backed content models plus YAML seed fixtures. Run Prisma schema sync and migrations from CI/CD with careful rollouts. Maintain a separate production DB and a staging DB seeded with anonymized or fixture data.
- Monitoring: expose health endpoints and integrate with Prometheus/Grafana or a managed APM. Add alerts for error rates, slow requests, worker crashes, and admin content pipeline failures.

---

## Where to look next

- Product & UX: `docs/` and `code-trainer-ux-docs/`
- Architecture & rationale: `CLAUDE.md`
- Generators: `scripts/`
- Curriculum: `content/javascript/`

If you want, I can open a branch with the generator + test fixes and add a short `DEPLOYMENT.md` runbook next. Which should I do first?
